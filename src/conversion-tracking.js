// Public source for TimeAds /t.js. The server validates every hit; the browser
// stores and echoes an opaque token and never creates or decrypts click IDs.
...
function createTracker() {
  ...
  let debug = !!script?.hasAttribute("data-debug");
  let endpoint = "";
  try { endpoint = new URL("/track", script.src).href; } catch { /* module: configure explicitly */ }
  let pendingLanding = null;

  function log(...args) {
    if (debug) window.console?.debug("[TimeAds]", ...args);
  }
  function load() {
  ...
  async function post(payload) {
    if (!endpoint) { log("Configure the tracking endpoint first."); return false; }
    try {
    ...
    } catch { log("Beacon request failed; call again to retry."); return false; }
  }
  async function land() {
    let token;
    try { token = new URLSearchParams(window.location.search).get(PARAM); }
    catch { return false; }
    if (!token) return false;
    let state = load();
    if (state?.token === token && state.landed) return true;
    if (pendingLanding?.token === token) return pendingLanding.promise;
    ...
    const promise = post({ click_id: token, transaction_id: state.transactionId, event: "landed" }).then(ok => {
      // A slower response from an earlier SPA navigation must not replace the
      // current visitor attribution. Retrying uses the same transaction ID.
      if (ok && load()?.token === token) save({ ...state, landed: true });
      if (pendingLanding?.token === token) pendingLanding = null;
      return ok;
    });
  ...
  async function track(transactionId, options = {}) {
    ...
    // Corrections are authenticated API-only. Never turn "update" into a
    // custom browser event, or imply that the browser can overwrite a value.
    ...
    if (options.value !== undefined) {
      ...
      // Decimal strings preserve the caller's precision across JSON decoding.
...
export default typeof window === "undefined" ? null : createTracker();
